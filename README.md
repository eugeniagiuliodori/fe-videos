...
El proyecto tiene como objetivo esencial ser un reproductor de videos.
Del lado del frontend, es una implementación con enfoque de jerarquía de capas, incluyendo un core de componentes UI reutilizables, para mantener consistencia y control interno del comportamiento y estilo.
En principio soporta una lista de videos youtube definidos estáticamente.
Cuenta con un control para gestionar funciones básicas de un video, como 'reproducir', 'pausar', 'ajustar volumen', 'ajustar velocidad de reproducción', 
...
(clicleando en el video, por encima del control visible propio de la aplicación), y de este modo navegar por videos aleatorios propuestos directamente por youtube. 
Los videos al momento estáticos propuestos por la APP se visualizan con scroll vertical.
Como potencialidad, se implementarán funcionalidades que impliquen comunicación API REST con backend, login con keycloack. Etc. Para los usuarios registrados en la aplicación, se permitirá el registro y/o autenticación a youtube, para soportar funcionalidades extras de youtube al video en reproducción.
