...
export interface ButtonPlayPauseProps extends ButtonProps {
  togglePlay:() => void;
  playingParent:boolean;
}

export interface ButtonMuteProps extends ButtonProps {
  toggleMute:() => void;
  volume:number;
  muted: boolean;
}
...
/* brand y theme activo */
export const DEFAULT_IDENTITY : UIIdentity = {
  brand: 'default' as const,
  theme: 'default-light' as const,
};

export const TOKENS_BY_IDENTITY: Record<Brand, Record<Theme, Tokens>> = {
  default: { 
    "default-light": lightTheme.tokens,
    "default-dark": darkTheme.tokens,
  },
}
...
DEFAULT_IDENTITY = estado actual

TOKENS_BY_IDENTITY = definición del dominio

ThemeVariables = renderización
*/
